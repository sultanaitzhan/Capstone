...
\rmk{Observe that one can decouple this system by further forcing 
\[
F_1(0) = F_2(0), \qquad F_2(0) - F_1(0)  = G_2(2\tau_0)  - G_1(2\tau_0)  
\]
...
\end{aligned}
\end{equation}
Note that all equations look like KdV except for the integral terms.
}

\subsubsection{Analysis of the system}
In this section, we analyse the system \eqref{HLSystem}. First, we switch back to $x,t$ coordinates and for simplicity consider the case $x<\tau_0. $Recalling the changes of variables, we have
\begin{align*}
x - \tau_0 &= \xi, \implies \partial_\xi = \partial_x \\
x + \tau_0 &= \zeta, \implies\partial_\zeta = \partial_x\\
y - \tau_0 &= \xi', \implies \D \xi' = \D y \\
y + \tau_0 &= \zeta', \implies \D \zeta' = \D y
\end{align*} 
Also, we have that $\partial_t = -\partial_\xi + \partial_\zeta + \epsilon \partial_{\tau_1},$ which implies $\partial_{\tau_1} = \frac{1}{\epsilon}(\partial_t + \partial_\xi - \partial_\zeta).$ 
